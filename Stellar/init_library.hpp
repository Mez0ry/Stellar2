...
template <typename _Ty,typename _InitTy,typename _DeinitTy, typename ... Ts>
class ILibrary {
protected:
  std::function<int(Ts ...)> m_init;
  std::function<void()> m_deinit;
private:
 _Ty* derived() {return static_cast<_Ty*>(this);}
public:
  ILibrary(_InitTy& init_cb,_DeinitTy& deinit_cb) : m_init(std::function<int(Ts ...)>(init_cb)), m_deinit(std::function<void()>(deinit_cb)){}
  virtual ~ILibrary() {}

  LibraryStatus Init(Ts&& ... args) { derived()->Init(args ...); }
...
public:
  ...
  init_library(Ts ... args) : m_Lib(std::make_unique<T>()),m_LibraryName(GetLibraryName<T>()), m_IsInitialized(false){
    if (!Logger::IsCreated()) {
      Logger::OnCreate();
    }
    
    STELLAR_ASSERT( is_sdl_library<T>::value, "template argument T is not and SDL library, it has type {0} ",typeid(decltype(m_Lib.get())).name());

    if constexpr ((std::is_invocable_v<decltype(&T::Init), T &, Ts...>)) {
