...
void Texture::SetPixelAt(uint8_t* pixels,uint32_t texture_width,const Vec2i& point,Color color){
  std::memcpy(&pixels[(point.x + texture_width * point.y) * sizeof(color)],&color,sizeof(color));
}

Color Texture::GetPixelColorAt(uint8_t* pixels, uint32_t texture_width,const Vec2i& point){
  Color color;
