...
public:
  ...
  Texture(const Core::Ref<Renderer> renderer, const std::string &texture_path,Deleter deleter = TextureDeleter()) {
  ...
  template <typename Deleter = TextureDeleter,
            class = enable_if_texture_constructible_t<Deleter>>
  Core::Ref<SDL_Texture> LoadTexture(const Core::Ref<Renderer> renderer,
                                     const std::string &path,
                                     Deleter deleter = TextureDeleter()) {
